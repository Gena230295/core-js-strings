...
function removeFirstOccurrences(str, value) {
  const newStr = str.indexOf(value);
  if (newStr === -1) {
    return str;
  }
  const str1 = str.slice(0, newStr);
  const str2 = str.slice(newStr + value.length, str.length);
  return str1 + str2;
}
...
function removeLastOccurrences(str, value) {
  const newStr = str.lastIndexOf(value);
  if (newStr === -1) {
    return str;
  }
  const str1 = str.slice(0, newStr);
...
function formatTime(minutes, seconds) {
  return `${minutes}:`.padStart(3, 0) + `${seconds}`.padStart(2, 0);
}
...
function extractNameFromTemplate(value) {
  const textStart = 'Hello, ';
  const newStr = value.slice(textStart.length, value.length - 1);
  return newStr;
}
